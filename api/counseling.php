...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $reqType = sanitize($input['request_type'] ?? 'named');
    $studentName = ($reqType === 'anonymous') ? 'Anonymous Student' : sanitize($input['student_name'] ?? ($user['name'] ?? 'Student'));
    $studentId = ($reqType === 'anonymous') ? 'Anonymous' : sanitize($input['student_id'] ?? ($user['student_id'] ?? 'DIT 14253'));
